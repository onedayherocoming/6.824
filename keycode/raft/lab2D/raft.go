...
type Raft struct {
	...
	lastSnapshotIndex int //快照中最后一个log的index
	lastSnapshotTerm  int //快照中最后一个log的term
...
func (rf *Raft) log(format string, a ...interface{}) {
	...
	//	rf.gid, rf.me, rf.role, rf.term, rf.commitIndex, rf.lastApplied, rf.matchIndex, rf.nextIndex, term, idx)
	//log.Printf("%s:log:%s\n", s, r)
	log.Printf("%s:\n",  r)
}
...
func Make(peers []*labrpc.ClientEnd, me int, persister *Persister, applyCh chan ApplyMsg, gid ...int) *Raft {
	...
	rf.voteFor = -1
	rf.role = Follower
	rf.logEntries = make([]LogEntry, 1) // idx ==0
