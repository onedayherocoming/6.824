...
func TestInitialElection2A(t *testing.T) {
	...
	if term1 != term2 {
		fmt.Printf("warning: term changed even though there were no failures :"+ strconv.Itoa(term1)+"  "+strconv.Itoa(term2))
	}
...
func TestReElection2A(t *testing.T) {
	...
	fmt.Println("将 "+strconv.Itoa(leader1)+" 网络连接断开")
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	fmt.Println("将 "+strconv.Itoa(leader1)+" 网络连接恢复")
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	fmt.Println("将 "+strconv.Itoa(leader2)+" 网络连接断开")
	fmt.Println("将 "+strconv.Itoa((leader2 + 1) % servers)+" 网络连接断开")
	...
	fmt.Println("将 "+strconv.Itoa((leader2 + 1) % servers)+" 网络连接恢复")
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	fmt.Println("将 "+strconv.Itoa(leader2)+" 网络连接恢复")
	cfg.connect(leader2)
	cfg.checkOneLeader()
